...
func generateDoc(commandDocFile string) error {
	...
	commandArray := []*cobra.Command{rootCmd, loginCmd, logoutCmd, listCmd, refreshCmd, onboardCmd, deactivateCmd}
...
var docsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		Debug.log("Running kabanero docs command.")
		err := generateDoc(docFile)
		if err != nil {
			Error.log("kabanero docs command failed with error: ", err)
			os.Exit(1)
		}
		Debug.log("kabanero docs command completed successfully.")
	},
}
var docFile = "test.mf"
